...
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QLabel, QGridLayout, QHBoxLayout, QFrame
...
"padding: 5px 5px;")
        ...
        icon5 = QtGui.QIcon()
        icon5.addPixmap(QtGui.QPixmap(":/blueIcons/resources/icons_blue/pen-tool.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.Profile_bttn.setIcon(icon5)
        ...
        icon6 = QtGui.QIcon()
        icon6.addPixmap(QtGui.QPixmap(":/blueIcons/resources/icons_blue/power.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
    ...
    def display_clients_sales_summary(self, clients_sales_summary_list):
        for i in reversed(range(self.verticalLayout_2.count())):
            self.verticalLayout_2.itemAt(i).widget().setParent(None)

        header_widget = QWidget()
        header_layout = QGridLayout(header_widget)
        header_layout.addWidget(QLabel("ClientID"), 0, 0)
        header_layout.addWidget(QLabel("Client Name"), 0, 1)
        header_layout.addWidget(QLabel("Total Sales"), 0, 2)
        self.verticalLayout_2.addWidget(header_widget)

        for row, client_sales_summary in enumerate(clients_sales_summary_list, start=1):
            client_widget = ClientSalesWidget(
                client_sales_summary['ClientID'],
                client_sales_summary['Username'],
                client_sales_summary['TotalSales'],
            )

            header_layout.addWidget(QLabel(str(client_sales_summary['ClientID'])), row, 0)
            header_layout.addWidget(QLabel(client_sales_summary['Username']), row, 1)
            total_sales_value = client_sales_summary['TotalSales'] or 0.0
            header_layout.addWidget(QLabel(f"${total_sales_value:.2f}"), row, 2)

        self.verticalLayout_2.addWidget(header_widget)


class ClientSalesWidget(QWidget):
    def __init__(self, client_id, client_name, total_sales, parent=None):
        super(ClientSalesWidget, self).__init__(parent)

        frame_layout = QHBoxLayout(self)
        frame = QFrame(self)
        frame.setFrameShape(QFrame.Box)  # Add a box frame around each row
        frame_layout.addWidget(frame)

        layout = QVBoxLayout(frame)
        layout.addWidget(QLabel(f"Client ID: {client_id}"))
        layout.addWidget(QLabel(f"Client Name: {client_name}"))
        total_sales_value = total_sales or 0.0
        layout.addWidget(QLabel(f"Total Sales: ${total_sales_value:.2f}"))